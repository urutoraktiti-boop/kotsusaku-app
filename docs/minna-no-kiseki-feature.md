...
### いちばん読み取りを使っているのは「設定画面」ではなく「1時間ごとの集計」

`aggregateAnalyticsSummaryCore()` は毎回 `analytics` を**全件読む**。
つまり **登録端末数 × 24回／日** が、利用者が誰も設定画面を開かなくてもかかる。

| 端末数 | 集計だけで1日にかかる読み取り |
|---|---|
| 500台 | 12,000 |
| 862台（2026-08-23時点） | **20,688** |
| 1,000台 | 24,000 |
| 2,000台 | 48,000（無料枠ほぼ満杯） |

**端末が増えるほど自動的に増える**ので、無料枠（1日50,000）が近づいてきたら
集計の間隔を2時間に延ばす（半分になる）か、前回から更新のあった端末だけ読む形に変える。
...
実データで確かめるコマンド（ブラウザ不要のREST）：

⚠️ **RESTでも読み取り回数はカウントされる**（以前ここに「読み取り課金なし」と書いてあったのは誤り）。
下の2つ目のコマンドは推移24件ぶんを読む。`analytics` を全件読むような調査を何度も回すと、
1回あたり端末数ぶん（いまなら862件）が加算されるので注意する。
